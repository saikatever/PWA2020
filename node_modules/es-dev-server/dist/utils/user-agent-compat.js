...
var browserslist = _interopRequireWildcard(require("browserslist"));

var caniuse = _interopRequireWildcard(require("caniuse-api"));

var _utils = require("./utils.js");

var _isModernBrowser = require("./is-modern-browser.js");
 ...
 */
const moduleFeatures = ['es6-module', 'es6-module-dynamic-import']; // Cache resolved user agents because we're going to request them a lot

/** @type {Map<string, UserAgentCompat>} */

const cache = new Map(); // Some browsers are resolved to a name unknown to browserslist
...
function normalizeSamsungVersion(browser, version) {
  try {
    browserslist(`${browser} ${version}`); // browserslist didn't throw, return the valid version

    return version;
  } catch (error) {
    // we gave an invalid version to browserslist, so we try to
    // find the nearest matching major version for samsung browser
    const validVersions = [...Object.keys(browserslist.versionAliases.samsung), ...browserslist.data.samsung.versions];
...
function calcUserAgentCompat(userAgent) {
  ...
  try {
    resolvedUA = (0, _browserslistUseragent.resolveUserAgent)(userAgent);
    const browserTarget = `${getBrowserName(resolvedUA.family)} ${getBrowserVersion(resolvedUA)}`;
    const modern = (0, _isModernBrowser.isModernBrowser)(userAgent);
    ...
    console.warn(`${message} Using max compatibility mode. Try updating your dependencies, ` + 'or file an issue with a reproduction.\n');
    return {
      browserTarget: null,
      modern: false,
      supportsEsm: false
...
function getUserAgentCompat(ctx) {
  ...
  if (!compat) {
    ...
    (0, _utils.logDebug)(`User agent: ${userAgent} detected as compatibility:`, compat);
  }
